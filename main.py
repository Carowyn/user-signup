...
class MainHandler(webapp2.RequestHandler):
    def get(self):
        ...
        user_content = "" # initialize user content
        ...
        error1_element = "<span class='error'>" + error1 + "</span>"

        error2 = self.request.get("space_error")
        error2_element = "<span class='error'>" + error1 + "</span>"

        error3 = self.request.get("invalid_user")
        error3_element = "<span class='error'>" + error3 + "</span>"

        user = "<p>" + user_label + user_input + "  " + error1_element + error2_element + error3_element + "</p>"
        ...
        error4_element = "<span class='error'>" + error4 + "</span>"

        error5 = self.request.get("invalid_password")
        error5_element = "<span class='error'>" + error5 + "</span>"

        password = "<p>" + password_label + password_input + "  " + error4_element + error5_element + "</p>"
        ...
        error6_element = "<span class='error'>" + error6 + "</span>"

        verify = "<p>" + verify_label + verify_input + "  " + error6_element + "</p>"

        # sets up the optional email field
        email_content = "" # initialize email content
        email_label = "<label>Email (optional): </label>"
        email_input = "<input type='text' name='email'>" + email_content + "</input>"

        error7 = self.request.get("space_in_email")
        error7_element = "<span class='error'>" + error7 + "</span>"

        error8 = self.request.get("invalid_email")
        error8_element = "<span class='error'>" + error8 + "</span>"

        email = "<p>" + email_label + email_input + "  " + error7_element + error8_element + "</p>"
...
class SignUp(webapp2.RequestHandler):
    ...
    def post(self):
        ...
        if user_escape == "":#error1
            ...
            self.redirect("/sign-up/no_username_error=" + no_username_error)
        elif " " in user_escape:#error2
            space_error = "Username cannot contain spaces.".format(user_escape)
            self.redirect("/sign-up/?space_error=" + space_error)
            ...
            self.redirect("/sign-up/?invalid_user=" + invalid_user)
        else:#valid
            user_valid = True
...
class Welcome(webapp2.RequestHandler):
    ...
    def post(self):
        ...
        welcome_content = page_header + welcome_element + page_footer
        self.response.write(content)
